...
% tree_CCR_ADA_train= (train_comatrix_tree(1,1) + train_comatrix_tree(2,2))/sum(sum(train_comatrix_tree));
...
% %test on test set
% label = predict(Mdl1,test_data);
% test_comatrix_tree = confusionmat(test_label,label);
% tree_CCR_ADA_test = (test_comatrix_tree(1,1) + test_comatrix_tree(2,2))/sum(sum(test_comatrix_tree));
...
% test on test set
label = predict(Mdl1,test_data);
test_comatrix_discriminant = confusionmat(test_label,label);
CCR_ADA_discriminant_test = (test_comatrix_discriminant(1,1) + test_comatrix_discriminant(2,2))/sum(sum(test_comatrix_discriminant));
